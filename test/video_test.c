...
FILE *f_stream3 = NULL;
void h264_cb3(const struct timeval *tv, const void *data,
	const int len, const int keyframe)
{
	static uint64_t count;
	int ret = 0;
	...
	ret = fwrite(data, sizeof(uint8_t), len, f_stream3);
	if (ret != len)
		printf("lost data when save data, write[%d] get[%d]\n",
			len, ret);
	printf("get [%llu] frames\n", ++count);
...
int main(int argc, char *argv[])
{
	...
	QCam_Video_Input_cb fun_cb[] = {h264_cb, h264_cb1, h264_cb2, h264_cb3};
	FILE *stream_file[4] = {f_stream, f_stream1, f_stream2, f_stream3};
	...
	while ((ch = getopt(argc, argv, "c:f:b:v:t:y:m:q:kisloh")) != -1) {
		...
		case 'h':
			...
			printf("\t-b set bitrate:2048,1024,768,512\n");
	...
	switch (chn.channelId) {
		...
		case 2:
			...
			break;
		case 3:
			chn.res = resolution[4];
			chn.bitrate = Stream_bitrate[4];
			break;
		default:
			chn.res = resolution[1];
			chn.bitrate = Stream_bitrate[1];					
	...
	if(mul_channel){
		...
		f_stream3 = fopen("out_3.h264","w+");
		if (f_stream == NULL || f_stream1 == NULL || 
			f_stream2 == NULL || f_stream3 == NULL) {
